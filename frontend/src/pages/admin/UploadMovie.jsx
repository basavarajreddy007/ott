...
export default function UploadMovie() {
  ...
  const handleGenerateDescription = useCallback(async () => {
    ...
    try {
      const { data } = await api.post("/ai/describe", {
        title: form.title,
        genre: genres.filter((g) => form.genres.includes(g._id)).map((g) => g.name).join(", "),
        year: form.releaseYear,
      });
      setForm((prev) => ({ ...prev, description: data.data.content }));
  ...
  }, [form.title, form.genres, form.releaseYear, genres]);
    ...
    <div className="um-page">
      ...
      <form onSubmit={handleSubmit} className="um-form">
        ...
        <Section title="Description" icon="📝">
          ...
          <button
            type="button"
            className={`um-ai-btn ${aiLoading ? "um-ai-btn--loading" : ""}`}
          ...
          <div className="um-toggles">
            {[["isFeatured", "⭐", "Featured"], ["isTrending", "🔥", "Trending"], ["isNewRelease", "🆕", "New Release"]].map(([name, icon, label]) => (
              <label key={name} className={`um-toggle ${form[name] ? "um-toggle--on" : ""}`}>
                ...
                <span className="um-toggle-switch">
        ...
        <div className="um-submit-row">
          <button type="button" className="um-btn-ghost" onClick={() => navigate(user?.role === "admin" ? "/admin" : "/movies")}>
            Cancel
          </button>
